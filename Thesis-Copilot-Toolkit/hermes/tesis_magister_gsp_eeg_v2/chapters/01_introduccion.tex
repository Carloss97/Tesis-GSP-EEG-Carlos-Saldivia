...
\begin{itemize}
    ...
    \item \textbf{OE2 — Evaluación multi-métrica y fisiológica.} Cuantificar el desempeño mediante seis métricas (MAE, RMSE, SNR, DTW, LSD, Coherencia) sobre tres conjuntos de datos y 120 escenarios de pérdida, y verificar el impacto en la preservación de ERPs (N100, P300) y de la estructura espectral en las bandas de frecuencia naturales del EEG.

    \item \textbf{OE3 — Trazabilidad y reproducibilidad.} Generar un repositorio abierto de artefactos computacionales —código, configuraciones, registros de optimización y resultados— que garantice la trazabilidad completa de cada afirmación cuantitativa hasta el archivo que la origina.
\end{itemize}

\section{Hipótesis de Trabajo}

\begin{quote}
\textbf{H1 — Hipótesis principal.} La regularización espacio-temporal (TRSS) permite reconstruir canales EEG faltantes con mayor precisión y robustez que los métodos geométricos, no solo en error de amplitud sino en la preservación de la morfología de ERPs y de la densidad espectral, especialmente bajo pérdida contigua severa.
\end{quote}

\begin{quote}
\textbf{H2 — Hipótesis de equivalencia condicional.} En regímenes favorables —montajes densos ($\geq 60$ canales), pérdida aleatoria $\leq 20\%$—, MNE-Python iguala o supera a TRSS en preservación espectral, siendo entre 50 y 100 veces más rápido sin un sacrificio clínico significativo.
\end{quote}
