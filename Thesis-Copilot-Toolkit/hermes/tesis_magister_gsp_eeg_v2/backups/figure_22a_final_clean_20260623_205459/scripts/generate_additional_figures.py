...
def fig_gsp_concepts() -> None:
    ...
    # (a) EEG graph; graph body and legend are vertically separated.
    ax = fig.add_subplot(gs[0, 0])
    clean_ax(ax)
    ax.set_aspect("equal")
    ax.set_title("(a) EEG como grafo", loc="left", fontweight="bold", pad=4)
    head_c = np.array([0.36, 0.55])
    original_c = np.array([0.50, 0.52])
    scale = 0.76
    add_head_outline(ax, head_c[0], head_c[1], 0.275, lw=1.25)
    pos = np.array([
        [0.42, 0.80], [0.58, 0.80],
    ...
    pos = (pos - original_c) * scale + head_c
    ...
    # Side legend: explanation and graph occupy separate regions.
    ax.text(0.68, 0.74, "Grafo EEG", transform=ax.transAxes,
            ha="left", va="center", fontsize=7.8, fontweight="bold", color="#111111")
    ax.scatter([0.70], [0.62], s=34, color="#F2C9A7", edgecolors="black",
               linewidth=0.45, transform=ax.transAxes, clip_on=False, zorder=5)
    ax.text(0.75, 0.62, "nodo =\nelectrodo", transform=ax.transAxes,
            ha="left", va="center", fontsize=7.4, color="#111111", zorder=5, linespacing=1.0)
    ax.plot([0.685, 0.735], [0.47, 0.47], color="#777777", lw=1.0,
            transform=ax.transAxes, clip_on=False, zorder=5)
    ax.text(0.75, 0.47, "arista =\ncercanía", transform=ax.transAxes,
            ha="left", va="center", fontsize=7.4, color="#111111", zorder=5, linespacing=1.0)
    ax.text(0.68, 0.30, "color = amplitud\nilustrativa", transform=ax.transAxes,
            ha="left", va="center", fontsize=7.4, color="#333333", zorder=5, linespacing=1.0)
