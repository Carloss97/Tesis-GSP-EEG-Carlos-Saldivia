...
def fig_gsp_concepts() -> None:
    ...
    head_c = np.array([0.50, 0.62])
    original_c = np.array([0.50, 0.52])
    scale = 0.82
    add_head_outline(ax, head_c[0], head_c[1], 0.30, lw=1.25)
    pos = np.array([
        [0.42, 0.80], [0.58, 0.80],
    ...
    # Compact graph legend in a dedicated lower band; nothing overlaps nodes.
    ax.add_patch(Rectangle((0.055, 0.015), 0.89, 0.175,
                           facecolor="white", edgecolor="#DDDDDD", lw=0.5,
                           transform=ax.transAxes, clip_on=False, zorder=2))
    ax.scatter([0.12], [0.145], s=34, color="#F2C9A7", edgecolors="black",
               linewidth=0.45, transform=ax.transAxes, clip_on=False, zorder=5)
    ax.text(0.17, 0.145, "nodo = electrodo", transform=ax.transAxes,
            ha="left", va="center", fontsize=7.5, color="#111111", zorder=5)
    ax.plot([0.12, 0.18], [0.090, 0.090], color="#777777", lw=0.9,
            transform=ax.transAxes, clip_on=False, zorder=5)
    ax.text(0.21, 0.090, "arista = cercanía espacial", transform=ax.transAxes,
            ha="left", va="center", fontsize=7.5, color="#111111", zorder=5)
    ax.text(0.12, 0.040, "color = amplitud ilustrativa sobre canales",
            transform=ax.transAxes, ha="left", va="center", fontsize=7.3, color="#333333", zorder=5)
