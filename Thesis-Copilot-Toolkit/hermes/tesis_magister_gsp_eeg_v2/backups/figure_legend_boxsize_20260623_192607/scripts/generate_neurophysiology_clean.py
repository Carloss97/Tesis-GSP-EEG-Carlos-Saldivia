...
def panel_origin(ax: plt.Axes) -> None:
    ...
    # Head cross-section only. Layer interpretation is in the caption, so no
    # text competes with the drawing inside the panel.
    cx, cy = 0.50, 0.52
    ...
    dipoles = [(0.45, 0.50, 70), (0.52, 0.47, 105), (0.56, 0.55, 55)]
    ...
    for x0 in [0.45, 0.50, 0.56]:
        ax.annotate("", xy=(x0, 0.80), xytext=(x0, 0.60),
                    arrowprops=dict(arrowstyle="->", color="#666666", lw=0.75, ls="--", alpha=0.70))

    # Legend below the drawing, not overlaid on anatomy.
    legend_item(ax, 0.05, 0.100, C["scalp"], "cuero cabelludo")
    legend_item(ax, 0.38, 0.100, C["skull"], "cráneo")
    legend_item(ax, 0.60, 0.100, C["csf"], "LCR")
    legend_item(ax, 0.05, 0.035, C["cortex"], "corteza")
    legend_item(ax, 0.38, 0.035, C["orange"], "dipolos", marker="line")
    legend_item(ax, 0.60, 0.035, C["blue"], "electrodos", marker="eeg")
