...
def fig_gsp_concepts() -> None:
    ...
    # Compact graph legend placed below the head, outside the node/edge drawing.
    ax.scatter([0.12], [0.070], s=34, color="#F2C9A7", edgecolors="black",
               linewidth=0.45, transform=ax.transAxes, clip_on=False, zorder=5)
    ax.text(0.17, 0.070, "nodo = electrodo", transform=ax.transAxes,
            ha="left", va="center", fontsize=7.6, color="#111111")
    ax.plot([0.52, 0.59], [0.070, 0.070], color="#777777", lw=0.9,
            transform=ax.transAxes, clip_on=False)
    ax.text(0.61, 0.070, "arista = cercanía", transform=ax.transAxes,
            ha="left", va="center", fontsize=7.6, color="#111111")
    ax.text(0.50, 0.025, "color: amplitud ilustrativa sobre canales",
            transform=ax.transAxes, ha="center", va="center", fontsize=7.4, color="#333333")
