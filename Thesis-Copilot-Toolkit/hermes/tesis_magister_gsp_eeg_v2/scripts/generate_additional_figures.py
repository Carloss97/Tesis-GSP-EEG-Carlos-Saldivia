...
def fig_gsp_concepts() -> None:
    ...
    # (a) EEG graph; clean graph body with a minimal bottom explanation.
    ax = fig.add_subplot(gs[0, 0])
    clean_ax(ax)
    ax.set_aspect("equal")
    ax.set_title("(a) EEG como grafo", loc="left", fontweight="bold", pad=4)
    head_c = np.array([0.50, 0.61])
    original_c = np.array([0.50, 0.52])
    scale = 0.80
    add_head_outline(ax, head_c[0], head_c[1], 0.29, lw=1.25)
    pos = np.array([
        [0.42, 0.80], [0.58, 0.80],
    ...
    # Minimal explanation below the drawing, no icons/boxes competing with graph.
    ax.text(0.50, 0.115, "Nodos = electrodos · Aristas = cercanía espacial",
            transform=ax.transAxes, ha="center", va="center",
            fontsize=7.5, color="#111111")
    ax.text(0.50, 0.055, "Color = amplitud ilustrativa sobre canales",
            transform=ax.transAxes, ha="center", va="center",
            fontsize=7.35, color="#444444")
