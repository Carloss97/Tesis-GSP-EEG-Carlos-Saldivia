# Integration Log — it30_knn_k5_3syn

**Decision**: GO ✓
**Date**: 2026-04-05
**Objective**: KNN k=5 graph on 3 synthetic datasets

## Go/No-Go

| Criterion | Result |
|-----------|--------|
| G1 MAE improvement | PASS |
| G2 RMSE improvement | PASS |
| G3 Significance | PASS |
| G4 CI95 non-degenerate | PASS |
| G5 QA gates | PASS |
| G6 Mandatory artefacts | PASS |
| G7 Error rate | PASS |

## Key Results

- Best method: **mean** (MAE = 0.0659)
- Best instant baseline MAE: 0.0659
- RMSE improvement: 27.4% at mr=0.2
- Significant contrast: rmse_tv_family_vs_instant (p=8.19e-05)