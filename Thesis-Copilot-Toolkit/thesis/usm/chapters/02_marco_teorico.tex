...
Para abandonar la restricción de la esfera euclidiana, el Procesamiento de Señales en Grafos (GSP) \cite{shuman_emerging_2013, ortega_graph_2018-1} propone modelar los electrodos y sus interacciones complejas como un grafo ponderado. 
...
\begin{itemize}
    \item \textbf{Grafos Geométricos:} Construidos \textit{a priori} usando la inversa de la distancia euclidiana entre las coordenadas 3D de los electrodos. Una variante popular es el Grafo K-Vecinos Más Cercanos Gaussianos ($k$-NN Gaussiano), donde $W_{ij} = \exp(-d(i,j)^2/\sigma^2)$ si $j$ está entre los $k$ vecinos de $i$, y $0$ en caso contrario. Tamaru et al. \cite{tamaru_optimizing_2024} han explorado variantes de $k$ dinámico para combatir irregularidades locales en la densidad de mallas.
    \item \textbf{Grafos Funcionales o de Correlación:} Extraídos empíricamente de la covarianza de la señal temporal o modelos adaptativos como \textit{Adaptive Edge Weighting} \cite{karasuyama_adaptive_2017}.
    \item \textbf{Aprendizaje de Grafos (Graph Learning):} Problemas de optimización inversos que buscan inferir la matriz $\mathbf{W}$ desde los datos pidiendo simultáneamente que la red resultante sea esparsa y que las señales observadas exhiban alta suavidad laplaciana \cite{kalofolias_how_2016, qiao_data-driven_2018}.
\end{itemize}
...
\section{Definición de las Métricas de Rendimiento}
...
\begin{itemize}
    \item \textbf{Mean Absolute Error (MAE):} Error lineal promedio que refleja la desviación absoluta media, altamente intuitivo para bioseñales.
    \begin{equation}
        \text{MAE}(\mathbf{x}, \hat{\mathbf{x}}) = \frac{1}{T} \sum_{t=1}^T | x(t) - \hat{x}(t) |
    \end{equation}
    
    \item \textbf{Root Mean Square Error (RMSE):} Medida de error cuadrática. Penaliza severamente grandes desviaciones aisladas (\textit{outliers}), sirviendo como proxy para inestabilidades en la interpolación.
    \begin{equation}
        \text{RMSE}(\mathbf{x}, \hat{\mathbf{x}}) = \sqrt{\frac{1}{T} \sum_{t=1}^T (x(t) - \hat{x}(t))^2}
    \end{equation}

    \item \textbf{Signal-to-Noise Ratio (SNR):} Cuantifica cuánta energía de la señal original es preservada frente a la energía del error residual, expresado en decibelios (dB). Un SNR alto indica una reconstrucción casi perfecta.
    \begin{equation}
        \text{SNR}(\mathbf{x}, \hat{\mathbf{x}}) = 10 \log_{10} \left( \frac{\sum x(t)^2}{\sum (x(t) - \hat{x}(t))^2} \right)
    \end{equation}

    \item \textbf{Dynamic Time Warping (DTW):} A diferencia del MAE o RMSE que comparan vectores punto a punto $\hat{x}(t)$ con $x(t)$, DTW busca la alineación temporal óptima de secuencias que minimice la distancia euclidiana. Es excepcionalmente valioso en EEG porque permite medir la fidelidad morfológica (la "forma" del ERP) sin ser excesivamente penalizado por minúsculos desfases de latencia de fase.

    \item \textbf{Log Spectral Distance (LSD):} Mide la distancia entre las Densidades Espectrales de Potencia (PSD) de ambas señales en escala logarítmica. Esto asegura que la reconstrucción preserve no solo la forma en el tiempo, sino la distribución de energía en las bandas Alfa, Beta y Gamma, crucial para análisis de BCI.
    
    \item \textbf{Magnitude-Squared Coherence (Coherencia):} Una medida entre $0$ y $1$ que indica el nivel de correlación de fase lineal entre ambas señales a través de distintas frecuencias.
\end{itemize}
