...
def main() -> None:
    ...
    print("  [✓] Datasets sintéticos: alpha, beta, broad")
    ...
    try:
        ...
        print(f"  [✓] PhysioNet EEGMMIDB real: {ds_physio['signals'].shape} "
              f"(sujetos {physionet_subjects_loaded})")
    except Exception as exc:
        print(f"  [✗] PhysioNet no disponible: {exc}")
        physionet_subjects_loaded = []
    ...
    if any(Path(bci_2a_path).glob("A*T.gdf")):
        try:
            ...
            print(f"  [✓] BCI Competition IV 2a: {ds_bci['signals'].shape}")
        except Exception as exc:
            print(f"  [✗] BCI Competition IV 2a falló (inesperado): {exc}")
    else:
        print(f"  [⚠] BCI Competition IV 2a: archivos .gdf NO encontrados en {bci_2a_path}")
