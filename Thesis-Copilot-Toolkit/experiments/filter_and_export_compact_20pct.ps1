...
$outdir = Join-Path $root 'filtered_results_20pct'
if (-not (Test-Path $outdir)) { New-Item -ItemType Directory -Path $outdir | Out-Null }

# Export CSVs
$out1_csv = Join-Path $outdir 'filtered_threshold_median_20pct.csv'
$out2_csv = Join-Path $outdir 'filtered_mean_realobs_20pct.csv'
...
$out_json = Join-Path $outdir 'filtered_selections_20pct.json'
...
$out_md = Join-Path $outdir 'filtered_selections_compact_20pct.md'
$sb = New-Object System.Text.StringBuilder
$sb.AppendLine('# Filtered selections (compact) - 20% cutoff') | Out-Null
