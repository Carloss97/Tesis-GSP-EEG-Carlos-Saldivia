...
# Exclude physionet dataset which shows anomalously small MAE values
exclude_ds = 'physionet_eegmmidb'
if 'dataset' in it05.columns:
    it05 = it05[it05['dataset'] != exclude_ds].copy()
...
min_datasets_for_generalist = max(3, int((it05['dataset'].nunique() if 'dataset' in it05.columns else 0) * 0.1))
...
# Write outputs (with suffix indicating physionet exclusion)
out_summary = out_dir / 'analysis_it05_it20_summary_no_physionet.csv'
full_agg = pd.concat([agg05.assign(experiment='it05_no_physionet'), agg20.assign(experiment='it20')], sort=False)
full_agg.to_csv(out_summary, index=False)

out_recs = out_dir / 'analysis_it05_it20_recommendations_no_physionet.json'
import json
...
report = []
report.append('Analysis IT05 + IT20 (excluding physionet) — resumen y recomendaciones')
report.append('---')
report.append('\n**Top 3 generalistas (it05 aggregated, physionet excluded)**')
for r in recommendations['generalists']:
    report.append(f"- {r['graph']} | {r['method']}: mean_mae={r['mean_mae']:.6f}, std={r['std_mae']:.6f}, datasets={int(r['datasets_count'])}, it20_mean_mae={r['it20_mean_mae']}")
...
report.append('\n**Baselines (aggregated it05, physionet excluded)**')
for r in recommendations['baselines']:
    report.append(f"- method={r['method']}: mean_mae={r['mean_mae']:.6f}, std={r.get('std_mae'):.6f}, best_graph={r.get('best_graph')}, best_graph_mean_mae={r.get('best_graph_mean_mae')}")
    if r.get('best_graph_top_datasets'):
        for d in r['best_graph_top_datasets']:
            report.append(f"    - {d['dataset']}: mae={d.get('mae')}")

report_path = out_dir / 'analysis_it05_it20_report_no_physionet.txt'
with open(report_path, 'w', encoding='utf-8') as f:
    f.write('\n'.join(report))

print('Done (excluded physionet).')
print(f'Wrote summary CSV: {out_summary}')
print(f'Wrote recommendations JSON: {out_recs}')
print(f'Wrote human report: {report_path}')

print('\n==== Short Summary ====')
print('Top generalists (no physionet):')
for r in recommendations['generalists']:
    print(f" - {r['graph']} | {r['method']} -> mean_mae={r['mean_mae']:.6f}")
print('Top temporal:')
for r in recommendations['temporal']:
    print(f" - {r['graph']} | {r['method']} -> mean_mae={r['mean_mae']:.6f}")
print('Top instant:')
