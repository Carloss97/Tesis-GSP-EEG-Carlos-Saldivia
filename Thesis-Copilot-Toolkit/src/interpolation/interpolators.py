...
def interpolate(method: str, signals_incomplete: np.ndarray, adjacency: np.ndarray, **kwargs) -> Dict[str, Any]:
    ...
    if method == "dummy_mean":
      ...
      isnan = np.isnan(signals_rec)
      means = np.nanmean(signals_rec, axis=1, keepdims=True)
      signals_rec[isnan] = np.take_along_axis(means, np.where(isnan)[1][None, :], axis=1).flatten()
