...
confirm_dir = ROOT / 'results_confirmatory_progressive_2026-04-16_06-10-16'
...
for entry in failed:
    ...
    # parse tag from iter name: confirm_prog_{idx}_{tag}
    ...
    try:
        ...
        if ds_key is None:
            print('Dataset not available for', ds)
            new_failed.append(entry)
            continue
        graphs = meta.get('graphs', [])
        graph_spec = ('knn', {'k': 3})
        if graphs:
            graph_spec = (graphs[0].split('__')[0], {})
