# HISTÓRICO — Diagnóstico de encaje editorial con Biomedical Signal Processing and Control

> **Documento reemplazado para la iteración activa.** Desde el 2026-07-23 el destino de trabajo es *IEEE Transactions on Biomedical Engineering* (TBME), el manuscrito se reconstruye desde cero a partir de la tesis y no se ejecutan experimentos nuevos. Consulte `docs/paper/ieee_tbme_thesis_protocol.md`, `docs/paper/ieee_tbme_claim_evidence_matrix.md` y `docs/plans/2026-07-23-plan-ieee-tbme-from-thesis.md`. Este archivo se conserva únicamente para trazabilidad histórica.
